...
import { DefensasMemoryRoutes } from './todos/routes.memory';
...
export class AppRoutes {
  ...
  static get routes(): Router {
    ...
    router.use('/api/defensas-memory', DefensasMemoryRoutes );
    ...
    router.get('/api/system/info', (req: Request, res: Response) => {
      res.json({
        ...
        endpoints: {
          ...
          memory: '/api/defensas-memory (always uses memory)',
    ...
    router.post('/api/system/datasource', (req: Request, res: Response) => {
      try {
        ...
        res.json({
          ...
          note: 'This change affects /api/todos endpoints. /api/defensas-memory always uses memory datasource.'
        });
      } catch (error) {
        res.status(500).json({
